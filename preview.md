...
| 기술 | 용도 |
|------|------|
| LangChain + LangGraph | 에이전트 워크플로우 |
| Ollama (학교 GPU 서버) | 로컬 LLM 서버 |
| gemma4:26b | 메인 대화 모델 |
| nomic-embed-text | 텍스트 임베딩 |
| ChromaDB | 벡터 DB (대화 메모리 + 가이드북) |

## 🛠 툴 시스템
| 툴 | 용도 |
|----|------|
| ddgs (DuckDuckGo) | 인터넷 실시간 검색 |
| memory_search | 과거 대화 맥락 조회 |
...
|------|------|
| Edge TTS | 텍스트 → 음성 변환 |
| Applio (RVC) | 음성 → 가온 목소리 변환 |
| PyAudio + SoundDevice | 오디오 출력 |
| VB-Cable | 가상 오디오 케이블 |
...
├── .env                     # 환경변수 (gitignore)
├── brain/
│   ├── agent.py             # LangGraph 에이전트 (think/answer 노드)
│   └── tools/
│       ├── __init__.py
│       ├── search.py        # 인터넷 검색 툴
│       ├── memory.py        # 대화 메모리 툴
...
## 🚀 실행 방법
1. VTube Studio 실행
2. `./start.sh` 실행 (Applio 자동 시작 + 에이전트 실행)
3. 방송 주제 입력
4. 치지직에서 방송 시작!
